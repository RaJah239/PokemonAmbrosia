...
; Johto story events
	...
	const EVENT_BILL_APPEARS_IN_VIOLET
	...
	const EVENT_GOT_LUCARIO_FROM_KIYO
	...
	const EVENT_GOT_WISE_GLASSES_IN_SLOWPOKE_WELL
...
; Johto hidden items
	...
	const EVENT_DRAGONS_DEN_B1F_AMBROSIA
	...
	const EVENT_ROUTE_44_HIDDEN_LUCKY_EGG
...
; Kanto story events
	...
	const EVENT_GOT_AMBROSIA_FROM_VERMILION_GUY
...
; DevNote - events, lets add more here
    ...
    const EVENT_BEAT_CHRIS_CHAN
    const EVENT_BEAT_MASTER_CHRIS_CHAN
    const EVENT_BEAT_MASTER_YAMI
    const EVENT_BEAT_MASTER_DRAKE
    const EVENT_BEAT_MASTER_EXECUTIVEF
    const EVENT_BEAT_TOBIAS
    const EVENT_BEAT_MASTER_TOBIAS
    const EVENT_BEAT_YUNA_1
    const EVENT_BEAT_YUNA_2
    const EVENT_BEAT_MASTER_YUNA
    const EVENT_BEAT_AIZEN
    const EVENT_BEAT_MASTER_AIZEN
    const EVENT_BEAT_XEHANORT
    const EVENT_BEAT_MASTER_XEHANORT
    const EVENT_BEAT_MASTER_SETO
    const EVENT_BEAT_MASTER_AERITH
    const EVENT_BEAT_MASTER_JOEY
    const EVENT_FRONTIER_INTRO
    const EVENT_BEAT_MASTER_DIANTHA
    const EVENT_BEAT_MASTER_ALDER
    const EVENT_BEAT_MASTER_LEON
    const EVENT_CAUGHT_GENESECT
    const EVENT_CERULEAN_CAVE_HIDDEN_AMBROSIA
    const EVENT_CERULEAN_CAVE_HIDDEN_MASTER_BALL
    ...
    const EVENT_BEAT_SOLDIER_6
    const EVENT_BEAT_SOLDIER_7
    const EVENT_BEAT_SOLDIER_8
    const EVENT_BEAT_SOLDIER_9
    const EVENT_BEAT_JULIUS
    ...
    const EVENT_TEMP_EVENT_2
    const EVENT_TEMP_EVENT_3
    ...
    const EVENT_DIGLETTS_CAVE_MUSCLE_BAND
    const EVENT_RUINS_OF_ALPH_INNER_CHAMBER_WISE_GLASSES
    const EVENT_SOUL_HOUSE_WISE_GLASSES
    const EVENT_CERULEAN_CAVE_HIDDEN_EXPERT_BELT
    const EVENT_CERULEAN_CAVE_HIDDEN_LIFE_ORB
    const EVENT_CERULEAN_CAVE_HIDDEN_LEFTOVERS
    const EVENT_CERULEAN_CAVE_HIDDEN_FOCUS_SASH
    const EVENT_CERULEAN_CAVE_HIDDEN_CHOICE_BAND
    ...
    const EVENT_CERULEAN_CAVE_HIDDEN_LUCKY_EGG
    const EVENT_CERULEAN_CAVE_HIDDEN_BRIGHT_POWDER
...
; Decorations
	...
	const EVENT_DECO_LUGIA_DOLL
	const EVENT_DECO_STARMIE_DOLL
	const EVENT_DECO_MAGIKARP_DOLL
	const EVENT_DECO_HO_OH_DOLL
	const EVENT_DECO_GENGAR_DOLL
	const EVENT_DECO_MR_MIMIE_DOLL
	const EVENT_DECO_DRAGONITE_DOLL
	const EVENT_DECO_ARCEUS_DOLL
	const EVENT_DECO_MEWTWO_DOLL
	const EVENT_DECO_UNOWN_DOLL
	const EVENT_DECO_GEODUDE_DOLL
	const EVENT_DECO_MACHOP_DOLL
	const EVENT_DECO_CROBAT_DOLL
...
; More Johto story events
	...
	const EVENT_BEAT_RIVAL_SAFFRON
...
; Camper
	...
	const EVENT_BEAT_INVADER_LAUTREC
...
; Pokemaniac
	...
	const EVENT_BEAT_INVADER_PATCHES
...
; Hiker
	...
	const EVENT_BEAT_INVADER_HAVEL
...
; Sailor
	const EVENT_BEAT_INVADER_BACKSTABER
...
; Johto itemballs
	...
	const EVENT_VIOLET_CITY_CHOICE_BAND
	const EVENT_VIOLET_CITY_RARE_CANDY
	const EVENT_LAKE_OF_RAGE_ELIXER
	const EVENT_LAKE_OF_RAGE_CHOICE_SPECS
	...
	const EVENT_UNION_CAVE_B1F_MUSCLE_BAND
	...
	const EVENT_ICE_PATH_B1F_LEFTOVERS
	...
	const EVENT_DARK_CAVE_RED_EYE_ORB
	...
	const EVENT_VICTORY_ROAD_LIFE_ORB
	const EVENT_VICTORY_ROAD_TM_FLAMETHROWER
	const EVENT_VICTORY_ROAD_AMBROSIA
	const EVENT_DRAGONS_DEN_B1F_DRAGON_FANG
	const EVENT_TOHJO_FALLS_POISON_BARB
	...
	const EVENT_ROUTE_43_MAX_ETHER ; not used
	const EVENT_ROUTE_44_LIFE_ORB
	const EVENT_ROUTE_44_ULTRA_BALL ; not used
	const EVENT_ROUTE_45_NUGGET
	const EVENT_ROUTE_45_ROCK_SLIDE
	const EVENT_ROUTE_45_ELIXER
	const EVENT_ROUTE_45_DOUBLE_EDGE
	const EVENT_ROUTE_46_PROTEIN
; Johto people
...
; New to Crystal
	...
	const EVENT_ICE_PATH_1F_CHOICE_SPECS
...
; Unused: next 48 events
    ...
    const EVENT_ARCEUS_APPEARS
	const EVENT_CAUGHT_ARCEUS
	const EVENT_CAUGHT_MEWTWO
	const EVENT_CAUGHT_KYOGRE
	const EVENT_CAUGHT_GROUDON
	const EVENT_CAUGHT_DIALGA
	const EVENT_CAUGHT_PALKIA
	const EVENT_CAUGHT_RAYQUAZA
	const EVENT_CAUGHT_GIRATINA
	const EVENT_CAUGHT_DARKRAI
	...
	const EVENT_JUST_BEAT_LORD_OAK
	const EVENT_NOT_USED
	const EVENT_BEAT_MASTER_RED
	const EVENT_BEAT_MASTER_LANCE
	const EVENT_BEAT_MASTER_BLUE
	const EVENT_BEAT_MASTER_GREEN
	const EVENT_BEAT_MASTER_BROCK
	const EVENT_BEAT_MASTER_MISTY
	const EVENT_BEAT_MASTER_SURGE
	const EVENT_BEAT_MASTER_ERIKA
	const EVENT_BEAT_MASTER_JANINE
	const EVENT_BEAT_MASTER_SABRINA
	const EVENT_BEAT_MASTER_BLAINE
	const EVENT_BEAT_MASTER_FALKNER
	const EVENT_BEAT_MASTER_BUGSY
	const EVENT_BEAT_MASTER_WHITNEY
	const EVENT_BEAT_MASTER_MORTY
	const EVENT_BEAT_MASTER_CHUCK
	const EVENT_BEAT_MASTER_JASMINE
	const EVENT_BEAT_MASTER_PRYCE
	const EVENT_BEAT_MASTER_CLAIR
	const EVENT_BEAT_MASTER_WILL
	const EVENT_BEAT_MASTER_BRUNO
	const EVENT_BEAT_MASTER_KAREN
	const EVENT_BEAT_MASTER_RIVAL
	const EVENT_BEAT_MASTER_ADAM
	...
	const EVENT_BEAT_INVADER_OROBORO
